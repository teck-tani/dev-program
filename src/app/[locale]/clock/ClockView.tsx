...
export default function ClockView() {
  ...
  return (
    <div className={`${styles.worldClockContainer} ${styles[state.theme]} ${isFullscreen ? styles.fullscreen : ''}`}>
      ...
      {/* SEO Content Section */}
      <section className={styles.seoSection} aria-label={locale === 'ko' ? '페이지 설명' : 'Page description'}>
        <h1>{locale === 'ko' ? '온라인 시계 - 초단위 정확한 서버시간' : 'Online Clock - Accurate Server Time'}</h1>
        
        <h2>{locale === 'ko' ? '정확한 시간이 필요할 때' : 'When You Need Precise Time'}</h2>
        <p>
          {locale === 'ko' 
            ? '티켓팅, 수능 시험, 중요한 마감 시간 등 초 단위까지 정확한 시간이 필요한 순간이 있습니다. 이 온라인 시계는 서버 시간을 기준으로 밀리초 단위까지 정밀하게 시간을 표시합니다. 디지털 세그먼트 스타일의 세련된 디자인으로, 한눈에 시간을 확인할 수 있으며 전 세계 70개 이상의 주요 도시 시간을 동시에 비교할 수 있습니다.'
            : 'There are moments when you need time accurate to the second - ticketing, exams, important deadlines. This online clock displays server time precisely to the millisecond. With its elegant digital segment design, you can check the time at a glance and compare times across 70+ major cities worldwide simultaneously.'}
        </p>

        <h2>{locale === 'ko' ? '세계 시간 한눈에 보기' : 'World Time at a Glance'}</h2>
        <p>
          {locale === 'ko' 
            ? '서울, 도쿄, 베이징, 뉴욕, 런던, 파리 등 전 세계 주요 금융 허브와 IT 중심지의 현재 시간을 확인하세요. 해외 출장, 국제 회의, 해외 거래 시 시차 계산이 필요할 때 유용합니다. 드래그 앤 드롭으로 자주 확인하는 도시를 원하는 순서로 정렬할 수 있습니다.'
        ...
        <ul>
          <li>{locale === 'ko' ? '초단위 정밀 서버시간 표시 - 티켓팅 시 정확한 타이밍을 위해' : 'Second-accurate server time - for precise timing during ticketing'}</li>
          <li>{locale === 'ko' ? '수능, 공무원 시험 등 중요 시험용 시계로 활용' : 'Ideal for important exams like SAT, civil service tests'}</li>
          <li>{locale === 'ko' ? '전 세계 70개 이상 주요 도시의 현재 시간 확인' : 'View current time in 70+ major cities worldwide'}</li>
          <li>{locale === 'ko' ? '도시 간 시차를 자동으로 계산하여 표시' : 'Automatically calculates and displays time differences between cities'}</li>
          <li>{locale === 'ko' ? '다크 모드 / 라이트 모드 테마 전환으로 눈의 피로 감소' : 'Dark/Light mode toggle to reduce eye strain'}</li>
          <li>{locale === 'ko' ? '드래그 앤 드롭으로 시계 순서 자유롭게 변경' : 'Freely reorder clocks with drag and drop'}</li>
          <li>{locale === 'ko' ? '글꼴 크기 조절로 가독성 향상' : 'Adjust font size for better readability'}</li>
          <li>{locale === 'ko' ? '전체화면 모드로 집중력 향상' : 'Fullscreen mode for better focus'}</li>
        </ul>

        <h2>{locale === 'ko' ? '활용 팁' : 'Usage Tips'}</h2>
        <p>
          {locale === 'ko' 
            ? '콘서트 티켓팅 시 정확한 시간에 예매를 진행하거나, 수능 시험 준비 시 실제 시험 환경처럼 시간을 체크해 보세요. 해외 주식 거래 시 각 증권거래소의 개장/폐장 시간을 확인하는 데도 유용합니다. 설정은 브라우저에 자동 저장되어 다음 방문 시에도 동일한 환경으로 사용할 수 있습니다.'
