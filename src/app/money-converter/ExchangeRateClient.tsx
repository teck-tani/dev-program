...
const getFlagUrl = (cur_unit: string) => {
    ...
    const countryCode = map[code] || "kr"; // Default to KR fallback? or just 'kr' if unknown to prevent break
...
export default function ExchangeRateClient() {
    ...
    // Changed CNY to CNH as Korea Exim Bank API uses CNH for Yuan
    ...
    useEffect(() => {
        if (!loading && rates.length > 0 && baseKrwValue === 0) {
            ...
            if (usdRate > 0) {
                setBaseKrwValue(usdRate); // 1 USD in KRW
            }
        }
    }, [loading, rates]);
                ...
                {loading ? (
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "15px" }}>
                        {rowCurrencies.map((code, idx) => {
                            ...
                            const curName = code === "KRW" ? "한국 원" : rate?.cur_nm || code;
